...
interface WaitlistProps {
  /**
   * One line, address only, no heading of its own.
   *
   * Used where the shop already owns the section and the list is the smaller
   * of two offers. Someone who was just shown a buy button and still wants the
   * list is doing us a favour, so it asks for the least it can and gets out of
   * the way.
   */
  compact?: boolean;
}
 ...
 */
export default function Waitlist({ compact = false }: WaitlistProps) {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    const result = await submitWaitlist({
      firstName: compact ? undefined : firstName.trim(),
      lastName: compact ? undefined : lastName.trim(),
      email: email.trim(),
      lang,
      company,
    });
  ...
  const nameRow = (
    /* Name row sits above the email so the fields read in the order they
       would be spoken. */
  ...
  /* With no name row above it, the email row draws its own top edge. */
  const edge = compact ? '' : 'border-t-0 ';

  const body = status === 'success' ? (
    <div
      className={compact ? 'border border-court px-4 py-3' : 'mt-8 max-w-[560px] border border-court p-5'}
      role="status"
    >
      <b className="label font-medium">{t.successTitle}</b>
      <p className={compact ? 'mt-1 text-[13px]' : 'mt-2 text-[13px]'}>{t.successBody}</p>
    </div>
  ) : (
    <>
      <form
        onSubmit={onSubmit}
        className={compact ? 'max-w-[440px]' : 'mt-8 max-w-[560px]'}
        data-reveal-item
        noValidate
      >
        {!compact && nameRow}
        ...
        {/*
          ...
          hidden fields but fill anything they can read. Kept out of the tab
          order and hidden from screen readers so no person meets it.
        */}
        ...
        <div className="flex flex-wrap">
          ...
          <input
            ...
            aria-invalid={Boolean(error)}
            aria-describedby={error ? 'wl-error' : undefined}
            className={`flex-[1_1_220px] border ${edge}border-court bg-cream px-4 ${
              compact ? 'py-3' : 'py-4'
            } font-mono text-[13px] tracking-[0.05em] outline-none focus:shadow-[inset_0_0_0_1px_theme(colors.court)]`}
          />
          <button
            type="submit"
            disabled={status === 'submitting'}
            className={`label border ${edge}border-court bg-court px-6 ${
              compact ? 'py-3' : 'py-4'
            } font-medium text-butter transition-colors hover:bg-cream hover:text-court disabled:opacity-60`}
          >
            {t.cta}
          </button>
        </div>
