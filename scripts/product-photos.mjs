...
const MANIFEST = join(ROOT, 'src/content/shop-photos.ts');
...
/*
 * Write the paths out as a module of string literals rather than letting the
 * app build them with `${name}-${width}.webp`.
 *
 * It keeps the widths that exist and the widths the app asks for from drifting
 * apart, which a template cannot: delete a rung here and the app stops
 * compiling instead of serving a 404. It is also what makes the shareable
 * single-file preview work, since that build inlines assets by matching quoted
 * path literals and has nothing to match against a string joined at runtime.
 */
const entries = PHOTOS.map(
  (p) =>
    `  '${p.name}': {\n` +
    WIDTHS.map((w) => `    ${w}: '/media/shop/${p.name}-${w}.webp',`).join('\n') +
    '\n  },',
).join('\n');
...
export const OUR_WIDTHS = [${WIDTHS.join(', ')}] as const;
